...
df.to_csv(dataCSVOutput, index=False, header=False)
df.to_excel(dataExcelOutput,index=False, header=False )

#Task 3 - Create a Database. Here's a SQLlite version. MySQL version under task 4

engine= create_engine('sqlite://', echo=False)
df.to_sql('lsa_single_line', con=engine, if_exists='replace')
engine.execute("SELECT * FROM lsa_single_line").fetchall()
print(engine.execute("SELECT * FROM lsa_single_line").fetchall())
engine.close()
